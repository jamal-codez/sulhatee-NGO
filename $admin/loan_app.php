    ...
    <div class="pagetitle">
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">Loan Applicant </li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="export-section" style="text-align: end;">
                            <form action='export.php' method='POST'>
                                    <input type="hidden" name="action" value="export_app_loan">
                                    <button class='btn btn-success' name='export_app_loan' type='submit'><span style='color:#ffff;'> <i class="bi bi-cloud-download-fill"> Download Record</i></span></button>
                            </form>
                        </div>
                        <br>
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            echo "<td>
                                                    <div class='btn-group gap-1' role='group'>
                                                        <a class='btn btn-primary' title='View' href='view_app.php?id=" . $row["id"] . "'><i class='bi bi-eye'>View</i></a>";
                                                        ...
                                                        if ($row["approve"] == 0) {
                                                            ...
                                                            echo "<a class='btn btn-success' title='Unapprove' href='./action/approve.php?id=" . $row["id"] . "&typ=user_bio&un=u' onclick='return confirmUnapprove()'><i class='bi bi-check'>Approved</i></a>";
                                                        }
        
                                                if ($row["suspention"] == 0) {
                                                    echo "<a class='btn btn-warning' title='Suspend' href='./action/suspend.php?id=" . $row["id"] . "&typ=user_bio&un=u2' onclick='return confirmSuspend()'><i class='bi bi-exclamation-triangle bi-2x'>Suspend</i></a>";
                                                } else {
                                                ...
                                                <button class='btn btn-danger' title='delete' type='submit' name='delete_app'><i
